...
locals {
  ...
  stage                          = var.stage
  cluster_name                   = data.terraform_remote_state.azure_tfstate.outputs.aks_cluster_name
  ...
  aks_cluster_client_key         = base64decode(data.azurerm_kubernetes_cluster.credentials.kube_config[0].client_key)
  aks_cluster_client_certificate = base64decode(data.azurerm_kubernetes_cluster.credentials.kube_config[0].client_certificate)
